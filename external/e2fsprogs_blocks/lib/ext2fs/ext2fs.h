...
#define MAX_BLK_LIST_ENTRIES (0x1000)

#pragma pack (push, block_mapping, 1)

typedef struct {
	__u64 location;    // in bytes
	__u32 len; // in bytes
} blk_list_entry;

typedef struct {
	blk_list_entry blk_list[MAX_BLK_LIST_ENTRIES];
	__u32 num_entries;
} blk_list;
#pragma pack (pop, block_mapping)
