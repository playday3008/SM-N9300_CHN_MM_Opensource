...
static int filefrag_fiemap(int fd, int blk_shift, int *num_extents,
			   ext2fs_struct_stat *st, blk_list* file_blk_list, int bs)
{
	char buf[16384];
	...
	if(NULL == file_blk_list){
		printf("filefrag_fiemap received null input");
		return -1;
	}
	...
	do {
		...
		if(fiemap->fm_mapped_extents > MAX_BLK_LIST_ENTRIES){
			printf("file has more mapped extents (%d) than expected (%d)", fiemap->fm_mapped_extents, MAX_BLK_LIST_ENTRIES);
			return -1;
		}
		...
		for (i = 0; i < fiemap->fm_mapped_extents; i++) {
			...
			file_blk_list->blk_list[i].location = (fm_ext[i].fe_physical >> blk_shift) * bs;
			file_blk_list->blk_list[i].len = (fm_ext[i].fe_length >> blk_shift) * bs;
	...
	*num_extents = tot_extents;
	file_blk_list->num_entries = tot_extents;
...
static void frag_report(const char *filename, blk_list* file_blk_list)
{
	...
	if (force_bmap ||
	    filefrag_fiemap(fd, blk_shift, &num_extents, &st, file_blk_list, fsinfo.f_bsize) != 0) {
...
int main(int argc, char**argv)
{
	char **cpp;
	int c;
	int i, count = 0, blk_count = 0;
	__u64 j;

	FILE *block_list_file;
	FILE *input_file_list;
	size_t written_objects, line_len;
	blk_list file_blk_list, blk_list_output;
	int read_bytes;
	__u32 ind;
	char * line = NULL;
	...
	input_file_list=fopen(argv[optind], "r+");
	if (NULL == input_file_list){
		exit(1);
	}

	block_list_file=fopen(argv[optind+1], "w");
	if (NULL == block_list_file){
		fclose(input_file_list);
		exit(1);
	}

	memset(&blk_list_output,0, sizeof(blk_list));

	while ((read_bytes = getline(&line, &line_len, input_file_list)) != -1 && line != NULL) {

		line[strlen(line) - 1] = 0;

		printf("file: %s\n", line);
		memset(&file_blk_list, 0, sizeof(blk_list));
		frag_report(line, &file_blk_list);

		if(file_blk_list.num_entries <= MAX_BLK_LIST_ENTRIES - blk_list_output.num_entries){
			for(ind = 0; ind < file_blk_list.num_entries; ind++){
				blk_list_output.blk_list[blk_list_output.num_entries + ind] = file_blk_list.blk_list[ind];
			}
			blk_list_output.num_entries += file_blk_list.num_entries;
			continue;
		} else {
			written_objects = fwrite(&blk_list_output, sizeof(blk_list), 1, block_list_file);
		if(written_objects < 1){
				printf("failed to write to output file\n");
				break;
			}
			blk_list_output = file_blk_list;
		}
		}
