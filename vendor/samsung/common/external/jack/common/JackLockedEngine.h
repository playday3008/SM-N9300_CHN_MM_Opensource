...
class SERVER_EXPORT JackLockedEngine
{
    ...
    public:
		...
		int ClientStopJackd(int refnum, int param)
        {
            TRY_CALL
            JackLock lock(&fEngine);
            return (fEngine.CheckClient(refnum)) ? fEngine.ClientStopJackd(refnum, param) : -1;
            CATCH_EXCEPTION_RETURN
        }

		int ClientSwitchMaster(int refnum, int type, int param)
        {
            TRY_CALL
            JackLock lock(&fEngine);
            return (fEngine.CheckClient(refnum)) ? fEngine.ClientSwitchMaster(refnum, type, param) : -1;
            CATCH_EXCEPTION_RETURN
        }
#endif
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
		int ClientStartDump(int refnum, int type, int param)
        {
            TRY_CALL
            JackLock lock(&fEngine);
            return (fEngine.CheckClient(refnum)) ? fEngine.ClientStartDump(refnum, type, param) : -1;
            CATCH_EXCEPTION_RETURN
        }

		int ClientStopDump(int refnum, int param)
        {
            TRY_CALL
            JackLock lock(&fEngine);
            return (fEngine.CheckClient(refnum)) ? fEngine.ClientStopDump(refnum, param) : -1;
            CATCH_EXCEPTION_RETURN
        }

		int ClientDump(int refnum, int param)
        {
            TRY_CALL
            JackLock lock(&fEngine);
            return (fEngine.CheckClient(refnum)) ? fEngine.ClientDump(refnum, param) : -1;
            CATCH_EXCEPTION_RETURN
        }
#endif
