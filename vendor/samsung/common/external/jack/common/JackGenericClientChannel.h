...
class JackGenericClientChannel : public detail::JackClientChannelInterface
{
    ...
    public:
		...
		void ClientStopJackd(int refnum, int param, int* result);
		void ClientSwitchMaster(int refnum, int type, int param, int* result);
#endif
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
		void ClientStartDump(int refnum, int type, int param, int* result);
		void ClientStopDump(int refnum, int param, int* result);
		void ClientDump(int refnum, int param, int* result);
#endif
